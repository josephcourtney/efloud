...
def rsync_failure_detail(results: JsonMapping) -> str | None:
    for name, raw in results.items():
        result = json_mapping_or_none(raw)
        if result is None or result.get("status") not in {"failed", "timed_out"}:
            continue
        detail = result.get("detail")
        return f"{name}: {detail}" if isinstance(detail, str) and detail else f"{name}: {result.get('status')}"
    return None
...
def _is_missing_remote_mmcif_bucket(*, source: RsyncSource, relative_path: str, result: OpResult) -> bool:
    if source.id != "pdb_mmcif" or result.status not in {"failed", "timed_out"}:
        return False
    if not _looks_like_mmcif_bucket_path(relative_path) or result.returncode != _RSYNC_FILE_OR_ATTR_ERROR_CODE:
        return False
...
def _normalize_path_result(source: RsyncSource, relative_path: str, result: OpResult) -> OpResult:
...
def _discover_mmcif_buckets(source: RsyncSource) -> set[str] | None:
    remote = f"{source.url.rstrip('/')}/mmCIF/"
    timeout_seconds = 1200
    command = ["rsync", "--list-only", f"--timeout={timeout_seconds}", f"--contimeout={timeout_seconds}"]
